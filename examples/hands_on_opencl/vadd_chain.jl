...
# for the kernel.
cl.call(queue, vadd, length(h_a), nothing, d_a, d_b, d_c, uint32(LENGTH))

# call the kernel again with different arguments
cl.call(queue, vadd, length(h_e), nothing, d_e, d_c, d_d, uint32(LENGTH))
cl.call(queue, vadd, length(h_g), nothing, d_g, d_d, d_f, uint32(LENGTH))
