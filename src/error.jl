...
const cl_err_desc = (Integer => String) [
    CL_INVALID_CONTEXT => "Context is not a valid context.",
    
    CL_INVALID_BUFFER_SIZE => "Buffer size is 0",
    
    CL_INVALID_EVENT => "Event objects specified in event_list are not valid event objects",
    
    CL_INVALID_HOST_PTR => string("If host_ptr is NULL and CL_MEM_USE_HOST_PTR or ",  
    ...
    CL_MEM_OBJECT_ALLOCATION_FAILURE => "Failure to allocate memory for buffer object.",
    ...
    CL_INVALID_BINARY => string("program is created with clCreateWithProgramBinary and devices listed in ",
    ...
    CL_BUILD_PROGRAM_FAILURE => string("Failure to build the program executable. ",
    ...
    CL_INVALID_KERNEL_DEFINITION => string("The function definition for __kernel  function ",
    ...
    CL_PROFILING_INFO_NOT_AVAILABLE => string("The CL_QUEUE_PROFILING_ENABLE flag ",
...
Base.show(io::IO, err::CLError) = Base.print_with_color(:red, io, "CLError($(err.code), $(err.desc))")
