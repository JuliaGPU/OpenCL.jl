...
Base.show(io::IO, p::Platform) = begin
...
let info_map = (Symbol => CL_platform_info) [
    ...
    function info(p::Platform, pinfo::Symbol)
        try
            ...
            pinfo == :extensions ? split(info(p, cl_info)) : info(p, cl_info)
        catch err
            if isa(err, KeyError)
...
devices(p::Platform, dtype::Symbol) = devices(p, cl_device_type(dtype))

devices(dtype::CL_device_type) = begin
    devs = Device[]
