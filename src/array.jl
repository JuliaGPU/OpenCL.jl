...
for (srcty, dstty) in [(:Array, :CLArray), (:CLArray, :Array), (:CLArray, :CLArray)]
    @eval begin
        function Base.unsafe_copyto!(
            ...
            nbytes = N * sizeof(T)
            return cl.enqueue_usm_memcpy(
                pointer(dst, dst_off), pointer(src, src_off), nbytes;
                blocking
            )
        end
...
function Base.unsafe_copyto!(
        ...
        dest::DenseCLArray{T}, doffs, src::Array{T}, soffs, n
    ) where {T}
...
function Base.unsafe_copyto!(
        ctx::cl.Context, dev::cl.Device,
        dest::Array{T}, doffs, src::DenseCLArray{T}, soffs, n
    ) where {T}
    GC.@preserve src dest unsafe_copyto!(ctx, dev, pointer(dest, doffs), pointer(src, soffs), n)
    if Base.isbitsunion(T)
        # copy selector bytes
...
function Base.unsafe_copyto!(
        ctx::cl.Context, dev::cl.Device,
        dest::DenseCLArray{T}, doffs, src::DenseCLArray{T}, soffs, n
    ) where {T}
    GC.@preserve src dest unsafe_copyto!(ctx, dev, pointer(dest, doffs), pointer(src, soffs), n)
    if Base.isbitsunion(T)
        # copy selector bytes
...
function Base.unsafe_copyto!(
        ...
        dest::DenseCLArray{T, <:Any, <:Union{cl.SharedBuffer, cl.HostBuffer}}, doffs, src::Array{T}, soffs, n
    ) where {T}
...
function Base.unsafe_copyto!(
        ctx::cl.Context, dev::cl.Device,
        dest::Array{T}, doffs, src::DenseCLArray{T, <:Any, <:Union{cl.SharedBuffer, cl.HostBuffer}}, soffs, n
    ) where {T}
    # maintain queue-ordered semantics
    synchronize(src)
