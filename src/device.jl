...
macro int_info(func, cl_device_info, return_type)
    quote
        function $(esc(func))(d::Device)
            ...
            api.clGetDeviceInfo(d.id, $cl_device_info,
...
function info(d::Device, s::Symbol)
    ...
    profile(d::Device) = begin
        size = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_PROFILE, 0, C_NULL, size)
        result = Vector{Cchar}(undef, size[])
        api.clGetDeviceInfo(d.id, CL_DEVICE_PROFILE, size[], result, C_NULL)
        bs = CLString(result)
        return bs
    end
    ...
    version(d::Device) = begin
        size = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_VERSION, 0, C_NULL, size)
        result = Vector{Cchar}(undef, size[])
        api.clGetDeviceInfo(d.id, CL_DEVICE_VERSION, size[], result, C_NULL)
        bs = CLString(result)
        return bs
    end
    ...
    driver_version(d::Device) = begin
        size = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DRIVER_VERSION, 0, C_NULL, size)
        result = Vector{Cchar}(undef, size[])
        api.clGetDeviceInfo(d.id, CL_DRIVER_VERSION, size[], result, C_NULL)
        bs = CLString(result)
    ...
    extensions(d::Device) = begin
        size = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_EXTENSIONS, 0, C_NULL, size)
        result = Vector{Cchar}(undef, size[])
        api.clGetDeviceInfo(d.id, CL_DEVICE_EXTENSIONS, size[], result, C_NULL)
        bs = CLString(result)
        return String[string(s) for s in split(bs)]
    end

    platform(d::Device) = begin
        result = Ref{api.cl_platform_id}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_PLATFORM,
                                   sizeof(api.cl_platform_id), result, C_NULL)
        return Platform(result[])
    end

    name(d::Device) = begin
        size = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_NAME, 0, C_NULL, size)
        result = Vector{Cchar}(undef, size[])
        api.clGetDeviceInfo(d.id, CL_DEVICE_NAME,
    ...
    device_type(d::Device) = begin
        result = Ref{api.cl_device_type}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_TYPE,
                                   sizeof(api.cl_device_type), result, C_NULL)
        result = result[]
    ...
    has_image_support(d::Device) = begin
        has_support = Ref{api.cl_bool}(CL_FALSE)
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE_SUPPORT,
                                   sizeof(api.cl_bool), has_support, C_NULL)
        return has_support[] == CL_TRUE
    end

    @int_info(queue_properties, CL_DEVICE_QUEUE_PROPERTIES, api.cl_command_queue_properties)
    ...
    has_native_kernel(d::Device) = begin
        result = Ref{api.cl_device_exec_capabilities}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_EXECUTION_CAPABILITIES,
                                   sizeof(api.cl_device_exec_capabilities), result, C_NULL)
    ...
    has_local_mem(d::Device) = begin
        result = Ref{api.cl_device_local_mem_type}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_LOCAL_MEM_TYPE,
                                   sizeof(api.cl_device_local_mem_type), result, C_NULL)
        return result[] == CL_LOCAL
    end

    host_unified_memory(d::Device) = begin
        result = Ref{api.cl_bool}(CL_FALSE)
        api.clGetDeviceInfo(d.id, CL_DEVICE_HOST_UNIFIED_MEMORY,
                                   sizeof(api.cl_bool), result, C_NULL)
        return result[] == CL_TRUE
    end

    available(d::Device) = begin
        result = Ref{api.cl_bool}(CL_FALSE)
        api.clGetDeviceInfo(d.id, CL_DEVICE_AVAILABLE,
                                   sizeof(api.cl_bool), result, C_NULL)
        return result[] == CL_TRUE
    end
    ...
    compiler_available(d::Device) = begin
        result = Ref{api.cl_bool}(CL_FALSE)
        api.clGetDeviceInfo(d.id, CL_DEVICE_COMPILER_AVAILABLE,
                                   sizeof(api.cl_bool), result, C_NULL)
        return result[] == CL_TRUE
    end
    ...
    max_work_item_size(d::Device) = begin
        dims = Ref{Cuint}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                                   sizeof(Cuint), dims, C_NULL)
        result = Vector{Csize_t}(undef, dims[])
        api.clGetDeviceInfo(d.id, CL_DEVICE_MAX_WORK_ITEM_SIZES,
    ...
    max_image2d_shape(d::Device) = begin
        width  = Ref{Csize_t}()
        height = Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE2D_MAX_WIDTH,
                                   sizeof(Csize_t), width,  C_NULL)
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE2D_MAX_HEIGHT,
    ...
    max_image3d_shape(d::Device) = begin
        ...
        height = Ref{Csize_t}()
        depth =  Ref{Csize_t}()
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE3D_MAX_WIDTH,
                                   sizeof(Csize_t), width, C_NULL)
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE3D_MAX_HEIGHT,
                                   sizeof(Csize_t), height, C_NULL)
        api.clGetDeviceInfo(d.id, CL_DEVICE_IMAGE3D_MAX_DEPTH,
...
function api.cl_device_type(dtype::Symbol)
    if dtype == :all
